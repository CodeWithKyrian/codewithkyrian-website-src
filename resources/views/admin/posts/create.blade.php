<x-admin-layout title="New Post">
    <x-admin.top-card title="Create a New Blog Post 😎" subtitle="Alright, it's time to get creative. Create something interesting!">
    </x-admin.top-card>
                ...
                <form class="px-5 py-2" enctype="multipart/form-data" action="{{ route('admin.posts.store') }}" method="POST">
                    ...
                    <div class="mt-4 text-gray-700" x-data="showImage()">
                        <div class="py-2">
                            ...
                            <input required type="text" name="title" value="{{ old('title') }}" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded focus:border-blue-500 focus:ring-0 focus:outline-none">
                        </div>
                        <div class="py-2">
                            <label class="font-medium" for="description">Description</label>
                            <textarea required rows="2" name="description" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded focus:border-blue-500 focus:ring-0 focus:outline-none">{{ old('description') }}</textarea>
                        </div>
                        <div class="py-2">
                            ...
                            <div class="flex">
                                <label for="is_draft" class="w-1/5">
                                    ...
                                    <div class="relative  my-2">
                                        <input id="is_draft" type="checkbox" type="checkbox" name="featured" class="hidden peer" x-model="is_draft" />
                                        <div class="bg-gray-200 w-20 h-10 rounded-full shadow-inner transition-all duration-300 ease-in-out peer-checked:bg-blue-500">
                                        </div>
                                        <div class="absolute top-1 left-1 w-8 h-8 bg-white rounded-full shadow transition-transform duration-300 inset-y-0 peer-checked:translate-x-[120%]">
                                        </div>
                                    </div>
                                </label>
                                <div class="w-2/5 px-2" x-show="!is_draft">
                                    <label class="font-medium" for="title">Posted At</label>
                                    <input required type="text" name="posted_at" value="{{ old('posted_at') }}" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded focus:border-blue-500 focus:ring-0 focus:outline-none">
                                </div>
                                <div class="w-2/5 px-2">
                                    <label class="font-medium" for="posted_at">Category</label>
                                    <select name="category_id" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded focus:border-blue-500 focus:ring-0 focus:outline-none">
                        ...
                        <div class="py-2">
                            ...
                            <textarea id="editor" name="content" class="prose block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded focus:border-blue-500 focus:ring-0 focus:outline-none">{{ old('content') }}</textarea>
                        </div>
                    </div>
